...
function preload() {
  ...
  wsound=loadSound("assets/cannon_water.mp3");
  cBlast=loadSound("assets/cannon_explosion.mp3");
  plaugh=loadSound("assets/pirate_laugh.mp3");
}
  ...
  if (boats.length > 0) {
    ...
    for (var i = 0; i < boats.length; i++) {
      ...
      if (collision.collided && !boats[i].isBroken) {
        if(!isLaughing&&!plaugh.isPlaying()){
            isLaughing=true;
            plaugh.play();
        }
